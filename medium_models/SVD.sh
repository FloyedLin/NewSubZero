...
RANK=${RANK:-4}
SVD_OPTIMIZER=${SVD_OPTIMIZER:-'sgd'}
BETA1=${BETA1:-0.9}
...
TYPE=prompt GRID_TAG=$GR_TAG TAG=$TAG STEPS=$STEP TASK=$TASK SEED=$SEED MODEL=$MODEL K=$K \
    bash run_fewshot_SVD.sh --per_device_train_batch_size $BS --learning_rate $LR --eval_steps $EVAL_STEP --weight_decay $WD --zo_eps $EPS \
    --zero_order_optim --lr_scheduler_type "constant" --optimizer "sgd" --efficient_zero_order \
    --SVD_optimizer $SVD_OPTIMIZER --beta1 $BETA1 --step_interval $STEP_INTERVAL --rank $RANK \
    $@
